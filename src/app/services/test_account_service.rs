...
mod test_account_service_find_all {
    ...
    fn test_find_all_accounts_non_empty_list() {
        ...
        account_repository
            .lock()
            .unwrap()
            .create(given_accounts.0.clone())
            .expect("Failed to create account");
        account_repository
            .lock()
            .unwrap()
            .create(given_accounts.1.clone())
            .expect("Failed to create account");
...
mod test_account_service_find_by_id {
    ...
    fn test_find_by_id_returns_a_value() {
        ...
        let account_id = account_repository
            .lock()
            .unwrap()
            .create(given_account.clone())
            .unwrap();
...
mod test_account_service_save {
    ...
    fn test_save_success() {
        ...
        let actual_account = account_repository
            .lock()
            .unwrap()
            .find_by_id(account_id.unwrap())
            .unwrap();
        assert_accounts_equal(&given_account, &actual_account, false);
    }
...
mod test_account_service_find_by_id_or_fail {
    ...
    fn test_find_by_id_returns_a_value() {
        ...
        let account_service = AccountService::new(account_repository);
        let account = account_service.find_by_id_or_fail(&account_id).unwrap();

        // THEN the account should be the same as the given account.
        assert_accounts_equal(&given_account, &account, false);
    }
